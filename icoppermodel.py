...
st.set_page_config(page_title= "Industrial Copper Modeling | By Meenakshi H",
                   layout= "wide",
                   initial_sidebar_state= "expanded")
image_link = "https://img.freepik.com/premium-photo/manufacturing-copper-wire-bronze-cable-reels-factory_1105604-532.jpg"
...
if opt=="HOME":
        ...
        st.write("#")
        st.write(f'<h4 style="color:white;">This project aims to develop two machine learning models for the copper industry to address the challenges of predicting selling price and lead classification.</h4>',unsafe_allow_html=True)
        st.write(f'<h4 style="color:white;">Manual predictions can be time-consuming and may not result in optimal pricing decisions or accurately capture leads.</h4>',unsafe_allow_html=True)
        st.write(f'<h4 style="color:white;">The models will utilize advanced techniques such as data normalization, outlier detection and handling, handling data in the wrong format, identifying the distribution of features, and leveraging tree-based models, specifically the decision tree algorithm, to predict the selling price and leads accurately.</h4>',unsafe_allow_html=True)
        st.write(" ")
        ...
        st.write("""
                     ### :orange[TECHNOLOGIES USED :] 
                        ...
                        - #### :white[PYTHON]
                        - #### :white[PANDAS]
                        - #### :white[DATA PREPROCESING]
                        - #### :white[EDA]
                        - #### :white[SCIKIT - LEARN]
                        - #### :white[STREAMLIT]""")
        
if opt=="PREDICT SELLING PRICE":
        st.write( f'<h4 style="color:orange;">NOTE: Min & Max given for reference, you can enter any value</h4>', unsafe_allow_html=True )
        ...
        if submit_button and flag==0:
            ...
            with open(r"reggression_model.pkl", 'rb') as file:
               loaded_model = pickle.load(file)
            with open(r'reggression_scaler.pkl', 'rb') as f:
               scaler = pickle.load(f)

            with open(r"reggressioni_type.pkl", 'rb') as f:
               item = pickle.load(f)

            with open(r"reggression_status.pkl", 'rb') as f:
                stat = pickle.load(f)
...
if opt=="PREDICT STATUS":
        ...
        if csubmit_button and cflag==0:
            import pickle
            with open(r"classify_model.pkl", 'rb') as file:
                cloaded_model = pickle.load(file)

            with open(r'classify_caler.pkl', 'rb') as f:
                cscaler_loaded = pickle.load(f)

            with open(r"classify_it.pkl", 'rb') as f:
